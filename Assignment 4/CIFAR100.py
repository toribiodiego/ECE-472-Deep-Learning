...
class ConfigManager:
    ...
    def __init__(self, config_path="100.yaml"):
...
class Trainer:
    ...
    @tf.function
    def train_step(self, batch_images, batch_labels, batch_size):
        with tf.GradientTape() as tape:
            model_output = self.model(batch_images, training=True)
            model_output = tf.reshape(model_output, [batch_size, -1])
            base_loss = tf.reduce_mean(
    ...
    def validate_epoch(self, val_images, val_labels, batch_size):
        ...
        for i in range(num_batches):
            ...
            all_predicted_probs.append(tf.nn.softmax(model_output).numpy())
            all_true_labels.append(np.argmax(batch_labels, axis=1))
        ...
        # Concatenate all the batch-wise predictions and true labels
        all_predicted_probs = np.concatenate(all_predicted_probs, axis=0)
        all_true_labels = np.concatenate(all_true_labels, axis=0)
        
        # Calculate top-5 accuracy
        top_5_accuracy = top_k_accuracy_score(all_true_labels, all_predicted_probs, k=5)
        
        avg_val_loss = total_val_loss / num_batches
        
        return avg_val_loss, top_5_accuracy
...
def main():
    ...
    batch_size = model_config.get("batch", 64)
